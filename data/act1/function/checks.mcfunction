...
# execute if entity @a[nbt=!{SelectedItem:{id:"minecraft:diamond",components:{"minecraft:custom_data":{door_key:true}}}}] run setblock 2 0 -252 oak_door[half=lower,facing=north,hinge=left,open=false] replace
# execute if entity @a[nbt=!{SelectedItem:{id:"minecraft:diamond",components:{"minecraft:custom_data":{door_key:true}}}}] run setblock 2 1 -252 oak_door[half=upper,facing=north,hinge=left,open=false] replace
